...
function displayTables(tables) {
    ...
    tables.forEach(name => {
        ...
        btn.onclick = () => {
            ...
            // Appel API pour créer la table dans Dynamics
            fetch(`/api/dynamics/create-table/${name}`, { method: 'POST' })
                .then(res => {
                    if (!res.ok) {
                        return res.json().then(errorData => {
                    ...
                    return res.json(); // Retourner la réponse JSON
                })
                .then(data => {
                ...
                .catch(err => {
                    // Gérer l'erreur et afficher un message d'échec
                    tdStatus.textContent = "❌ Échec du chargement";
                    console.error("Erreur lors du chargement des données vers Dynamics:", err);
                    btn.disabled = false;
                    btn.textContent = retryText; // Permet de réessayer
                });
        };
...
// Fonction pour récupérer les tables SQL et les afficher dans le tableau
function fetchAndDisplayTables() {
    fetch('/api/chargement/tables') // Requête API pour récupérer les tables SQL
        .then(res => {
            if (!res.ok) {
                throw new Error(`Erreur HTTP : ${res.status}, ${res.statusText}`);
            }
            return res.json(); // Retourne le JSON contenant les noms des tables
        })
        .then(tables => {
            displayTables(tables); // Afficher les tables dans le tableau HTML
            const tablesContainer = document.getElementById('tablesContainer');
            tablesContainer.classList.remove('d-none'); // Afficher le conteneur des tables
        })
        .catch(err => {
            console.error("Erreur lors du chargement des tables SQL", err);
            alert(`Erreur lors du chargement des tables. Détails de l'erreur : ${err.message}`);
